...
const projects: Project[] = [
  {
    ...
    longDescription: {
      pl: "Platforma czatu sportowego z analizą meczów wspieraną przez AI. Czat na żywo, przewidywania AI, statystyki piłkarskie w czasie rzeczywistym. Zbudowana z Next.js 15 App Router, platforma oferuje czat na żywo podczas meczów i przed meczami z wykorzystaniem Socket.IO, prywatną komunikację między użytkownikami oraz wielojęzyczne wsparcie (PL/EN). Integracja z OpenAI zapewnia szczegółową analizę meczów, przewidywania z formatem double chance oraz inteligentne limity dzienne z kontrolą kosztów. Aplikacja wykorzystuje dane piłkarskie z Redis caching, co redukuje wywołania API o 60-80%. Funkcje obejmują paginację 50 meczów na stronę, wyszukiwanie pełnotekstowe, interaktywne widgety ze statystykami meczów oraz dynamiczny wybór daty.",
      en: "Live sports chat platform with AI-powered match analysis. Real-time chat, AI predictions, live football statistics. Built with Next.js 15 App Router, the platform offers live chat during and before matches using Socket.IO, private user-to-user communication, and multi-language support (PL/EN). OpenAI integration provides detailed match analysis, predictions with double chance format, and smart daily limits with cost control. The app uses football data with Redis caching, reducing API calls by 60-80%. Features include pagination of 50 matches per page, full-text search, interactive widgets with match statistics, and dynamic date selection.",
    },
    technologies: [
      { name: "Next.js" },
      { name: "React" },
  ...
  },
  {
    ...
    longDescription: {
      pl: "Nowoczesna, zoptymalizowana pod SEO strona wizytówkowa dla hurtowni naturalnych słodzików działającej w Polsce i UE. Wielojęzyczna, z zaawansowaną ochroną antyspamową i optymalizacją wydajności. Strona prezentuje hurtową ofertę naturalnych słodzików, w tym Stevia Rebaudiana Bertoni, Erythritol, Xylitol, Sucralose, Sorbitol i inne alternatywy cukru. Zoptymalizowana pod kątem SEO z czystymi URL-ami (bez rozszerzeń .html), automatycznie aktualizowanym sitemap.xml, znacznikami Schema.org, meta tagami i hreflang dla wsparcia dwujęzycznego (PL/EN). Formularz kontaktowy zawiera wielowarstwową ochronę antyspamową: pole honeypot, walidację timestamp oraz walidację po stronie serwera. Strona wykorzystuje obrazy WebP, zoptymalizowane czcionki, minifikację CSS, kompresję Gzip i cache przeglądarki dla statycznych zasobów. Pełna responsywność i wsparcie dla wszystkich nowoczesnych przeglądarek.",
      en: "Modern, SEO-optimized business card website for a natural sweeteners wholesale company operating in Poland and the EU. Multilingual, with advanced anti-spam protection and performance optimization. The website showcases wholesale natural sweeteners including Stevia Rebaudiana Bertoni, Erythritol, Xylitol, Sucralose, Sorbitol, and other sugar alternatives. SEO-optimized with clean URLs (no .html extensions), auto-updated sitemap.xml, Schema.org markup, meta tags, and hreflang tags for bilingual support (PL/EN). The contact form includes multi-layer spam protection: honeypot field, timestamp validation, and server-side validation. The site uses WebP images, optimized fonts, CSS minification, Gzip compression, and browser caching for static assets. Fully responsive with support for all modern browsers.",
    },
    technologies: [
      { name: "HTML" },
      { name: "CSS" },
  ...
  },
  {
    ...
    longDescription: {
      pl: "Kompleksowy system SaaS do zarządzania czasem pracy i urlopami dla przedsiębiorstw. Śledzenie czasu, zarządzanie urlopami, koordynacja zespołu z architekturą multi-tenant i kontrolą dostępu opartą na rolach. Planopia składa się z dwóch głównych komponentów: zoptymalizowanych pod SEO stron marketingowych (Next.js 15) z blogiem i formularzami kontaktowymi oraz aplikacji biznesowej (React 18) do śledzenia czasu pracy i zarządzania urlopami. System oferuje darmowy plan dla do 8 użytkowników, architekturę multi-tenant z izolacją danych dla każdego zespołu, kontrolę dostępu opartą na rolach (Admin, HR, Supervisor, Worker), zautomatyzowane przepływy pracy dla procesów HR oraz wsparcie wielojęzyczne (PL/EN). Funkcje obejmują codzienne logowanie czasu z nadgodzinami, widok kalendarza miesięcznego, eksport PDF raportów, system wniosków urlopowych z zatwierdzaniem, planowanie urlopów w kalendarzu zespołu oraz zarządzanie użytkownikami i działami. Aplikacja wykorzystuje JWT do autentykacji, rate limiting dla ochrony API, szyfrowanie haseł z bcrypt oraz audit logging dla zgodności. Pełna responsywność i wsparcie dla wszystkich urządzeń.",
      en: "Comprehensive SaaS solution for enterprise time tracking, leave management, and team coordination with multi-tenant architecture and role-based access control. Planopia consists of two main components: SEO-optimized marketing pages (Next.js 15) with blog and contact forms, and a business application (React 18) for time tracking and leave management. The system offers a free plan for up to 8 users, multi-tenant architecture with isolated data for each team, role-based access control (Admin, HR, Supervisor, Worker), automated workflows for HR processes, and multi-language support (PL/EN). Features include daily time logging with overtime, monthly calendar view, PDF export for reports, leave request system with approvals, team leave planning calendar, and user and department management. The app uses JWT authentication, rate limiting for API protection, password hashing with bcrypt, and audit logging for compliance. Fully responsive with support for all devices.",
    },
    technologies: [
      { name: "React" },
      { name: "Next.js" },
